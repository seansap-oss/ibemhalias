...
export function CmsContentManager() {
  ...
  const loadItems = React.useCallback(async () => {
    const response = await fetch(`/api/cms/content?section=${encodeURIComponent(section)}`);
  ...
  const publish = async () => {
    ...
    try {
      ...
      if (!createResponse.ok) {
        throw new Error(createPayload.error || "Unable to publish");
      }
      ...
      setMessage(error?.message || "Unable to publish.");
    } finally {
      setPublishing(false);
    }
  };
  ...
  return (
    <div className="grid min-h-[720px] gap-5 xl:grid-cols-[260px_1fr_390px]">
          ...
          {CMS_SECTION_GROUPS.map((group) => (
            <div key={group}>
              ...
              <div className="space-y-1">
                {CMS_SECTIONS.filter((item) => item.group === group).map((item) => (
                  <button
                    key={item.path}
      ...
      <section className="min-w-0">
        <div className="rounded-[26px] border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <h2 className="mt-2 text-2xl font-black text-slate-950">
            {CMS_SECTIONS.find((item) => item.path === section)?.label || section}
          </h2>
          <p className="mt-1 break-all text-xs text-slate-500">{section}</p>
        </div>

        <div className="mt-4 overflow-hidden rounded-[26px] border border-slate-200 bg-white shadow-sm">
          <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
            <h3 className="font-black text-slate-950">Published Content</h3>
                ...
                <div key={item.id} className="flex items-center gap-3 p-4">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-1 flex flex-wrap gap-x-3 gap-y-1 text-[11px] text-slate-500">
                      ...
                      {!!item.month_label && <span>{item.month_label}</span>}
                    </div>
                  </div>
      ...
      <aside className="rounded-[26px] border border-slate-200 bg-white p-5 shadow-sm">
        <h3 className="text-xl font-black text-slate-950">Upload / Publish</h3>
        ...
        <div className="mt-5 grid grid-cols-2 gap-2 rounded-2xl bg-slate-100 p-1">
          ...
          <button
            ...
            className={[
              "rounded-xl px-3 py-2 text-xs font-black",
              mode === "youtube" ? "bg-white text-indigo-700 shadow-sm" : "text-slate-500",
            ].join(" ")}
          >
            YouTube URL
          </button>
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          <div>
            <label className="block text-xs font-black text-slate-700">Date Label</label>
            <input
              value={dateLabel}
          ...
          <div>
            <label className="block text-xs font-black text-slate-700">Month Label</label>
            <input
              value={monthLabel}
        ...
        {mode === "file" ? (
          <label className="mt-5 flex min-h-36 cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed border-indigo-200 bg-indigo-50/40 p-4 text-center">
            ...
            <div className="mt-3 text-sm font-black text-slate-900">
              {file ? file.name : "Tap or click to choose file"}
            </div>
