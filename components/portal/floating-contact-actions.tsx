...
export function FloatingContactActions({
  ...
  React.useEffect(() => {
    try {
      ...
      if (saved === "right" || saved === "left") setSide(saved);
    } catch {}
  }, []);

  const isReadingOrContentPage = pathname !== "/";
  const opacityClass = isReadingOrContentPage ? "opacity-70" : "opacity-100";

  const saveSide = (next: "left" | "right") => {
    setSide(next);
    try {
  ...
  const onDragEnd = (_: MouseEvent | TouchEvent | PointerEvent, info: PanInfo) => {
    if (Math.abs(info.offset.x) < 25) return;
    saveSide(info.offset.x > 0 ? "right" : "left");
  };
  ...
  return (
    ...
    >
      ...
      <a
        ...
        aria-label="WhatsApp Ibemhal IAS"
        onPointerDown={(event) => event.stopPropagation()}
